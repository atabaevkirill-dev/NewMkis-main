"""
Protocol handler for the 3km eye-safe laser rangefinder module
Implements the communication protocol as described in the specification
"""
...
class ProtocolHandler:
    """Handles communication with the laser rangefinder module"""
    ...
    def __init__(self, comm_port):
        """
        Initialize the protocol handler with a communication port object
        :param comm_port: pyserial.Serial object or socket.socket object
        """
        self.comm_port = comm_port
        # Determine if this is a serial port or socket
        self.is_serial = isinstance(comm_port, serial.Serial)
        self.is_socket = hasattr(comm_port, 'send') and hasattr(comm_port, 'recv')
    ...
    def send_command(self, command_code, params=None):
        """
        Send a command to the rangefinder module
        :param command_code: Command code byte
        :param params: List of parameter bytes (optional)
        ...
        try:
            ...
            if self.is_serial:
                self.comm_port.write(bytes(packet))
            elif self.is_socket:
                self.comm_port.send(bytes(packet))
            else:
                raise Exception("Communication port is neither serial nor socket")
            return True
        except Exception as e:
            print(f"Error sending command {command_code:02X}: {str(e)}")
            return False
    
    def read_response(self, timeout=2.0):
        """
        Read a response packet from the rangefinder module
        :param timeout: Timeout in seconds
        :return: Response packet as list of bytes, or None if error/timeouts
        """
        if self.is_serial:
            return self._read_from_serial(timeout)
        elif self.is_socket:
            return self._read_from_socket(timeout)
        else:
            print("Communication port is neither serial nor socket")
            return None
    
    def _read_from_serial(self, timeout):
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if self.comm_port.in_waiting >= 3:  # At least header + length
                    # Look for frame header (0xEE 0x16)
                    header = self.comm_port.read(2)
                    if header[0] == 0xEE and header[1] == 0x16:
                        # Read data length
                        length_byte = self.comm_port.read(1)[0]
                        
                        # Read the remaining bytes (device code + command code + params + checksum)
                        remaining_bytes = self.comm_port.read(length_byte + 1)  # +1 for checksum
                        
                        if len(remaining_bytes) == length_byte + 1:
                            # Return complete packet
                            packet = [0xEE, 0x16, length_byte] + list(remaining_bytes)
                            print(f"Complete packet received: {[f'{b:02X}' for b in packet]}")
                            return packet
                    else:
                        # Not the header we're looking for, skip this byte
                        continue
                
                time.sleep(0.01)  # Small delay to prevent busy-waiting
            except Exception as e:
                print(f"Error reading response from serial: {str(e)}")
                return None
        
        print("Timeout waiting for response from serial")
        return None
    
    def _read_from_socket(self, timeout):
        """Read response from socket"""
        try:
            self.comm_port.settimeout(timeout)
            
            # Read at least the header (0xEE 0x16) and length byte
            header_and_length = self.comm_port.recv(3)
            ...
            remaining_bytes = self.comm_port.recv(length_byte + 1)
