...
class PelcoDController(QWidget):
    """
    Контроллер управления устройством по протоколу Pelco-D
    """
    ...
    def __init__(self, parent=None):
        ...
        self.tcp_ip = config.get("tcp_ip", "192.168.127.254")
        self.tcp_port = config.get("tcp_port", 9762)  # Порт 9762 для протокола Pelco-D
    ...
    def setup_ui(self):
        ...
        # Заголовок
        title_label = QLabel("Контроллер Pelco-D для устройства RelayX3")
        ...
        # Группа таймеров
        timer_group = QGroupBox("Таймеры каналов")
        timer_layout = QVBoxLayout()
        
        # Настройки таймера для канала 1
        channel1_timer_layout = QHBoxLayout()
        channel1_timer_layout.addWidget(QLabel("Канал 1 - Время (с):"))
        self.channel1_time_spinbox = QSpinBox()
        self.channel1_time_spinbox.setRange(1, 3600)  # 1 секунда до 1 часа
        self.channel1_time_spinbox.setValue(5)  # По умолчанию 5 секунд
        channel1_timer_layout.addWidget(self.channel1_time_spinbox)
        
        self.channel1_timer_start_btn = QPushButton("Запустить таймер")
        self.channel1_timer_start_btn.clicked.connect(lambda: self.start_timer("left"))
        self.channel1_timer_start_btn.setEnabled(False)
        
        self.channel1_timer_stop_btn = QPushButton("Остановить таймер")
        self.channel1_timer_stop_btn.clicked.connect(lambda: self.stop_timer("left"))
        self.channel1_timer_stop_btn.setEnabled(False)
        
        channel1_timer_layout.addWidget(self.channel1_timer_start_btn)
        channel1_timer_layout.addWidget(self.channel1_timer_stop_btn)
        timer_layout.addLayout(channel1_timer_layout)
        
        # Настройки таймера для канала 2
        channel2_timer_layout = QHBoxLayout()
        channel2_timer_layout.addWidget(QLabel("Канал 2 - Время (с):"))
        self.channel2_time_spinbox = QSpinBox()
        self.channel2_time_spinbox.setRange(1, 3600)  # 1 секунда до 1 часа
        self.channel2_time_spinbox.setValue(5)  # По умолчанию 5 секунд
        channel2_timer_layout.addWidget(self.channel2_time_spinbox)
        
        self.channel2_timer_start_btn = QPushButton("Запустить таймер")
        self.channel2_timer_start_btn.clicked.connect(lambda: self.start_timer("right"))
        self.channel2_timer_start_btn.setEnabled(False)
        
        self.channel2_timer_stop_btn = QPushButton("Остановить таймер")
        self.channel2_timer_stop_btn.clicked.connect(lambda: self.stop_timer("right"))
        self.channel2_timer_stop_btn.setEnabled(False)
        
        channel2_timer_layout.addWidget(self.channel2_timer_start_btn)
        channel2_timer_layout.addWidget(self.channel2_timer_stop_btn)
        timer_layout.addLayout(channel2_timer_layout)
        
        # Настройки таймера для канала 3
        channel3_timer_layout = QHBoxLayout()
        channel3_timer_layout.addWidget(QLabel("Канал 3 - Время (с):"))
        self.channel3_time_spinbox = QSpinBox()
        self.channel3_time_spinbox.setRange(1, 3600)  # 1 секунда до 1 часа
        self.channel3_time_spinbox.setValue(5)  # По умолчанию 5 секунд
        channel3_timer_layout.addWidget(self.channel3_time_spinbox)
        
        self.channel3_timer_start_btn = QPushButton("Запустить таймер")
        self.channel3_timer_start_btn.clicked.connect(lambda: self.start_timer("third"))
        self.channel3_timer_start_btn.setEnabled(False)
        
        self.channel3_timer_stop_btn = QPushButton("Остановить таймер")
        self.channel3_timer_stop_btn.clicked.connect(lambda: self.stop_timer("third"))
        self.channel3_timer_stop_btn.setEnabled(False)
        
        channel3_timer_layout.addWidget(self.channel3_timer_start_btn)
        channel3_timer_layout.addWidget(self.channel3_timer_stop_btn)
        timer_layout.addLayout(channel3_timer_layout)
        
        # Кнопки для управления всеми таймерами
        all_timers_layout = QHBoxLayout()
        
        self.start_all_timers_btn = QPushButton("Запустить все таймеры")
        self.start_all_timers_btn.clicked.connect(self.start_all_timers)
        self.start_all_timers_btn.setEnabled(False)
        
        self.stop_all_timers_btn = QPushButton("Остановить все таймеры")
        self.stop_all_timers_btn.clicked.connect(self.stop_all_timers)
        self.stop_all_timers_btn.setEnabled(False)
        
        all_timers_layout.addWidget(self.start_all_timers_btn)
        all_timers_layout.addWidget(self.stop_all_timers_btn)
        all_timers_layout.addStretch()
        
        timer_layout.addLayout(all_timers_layout)
        ...
        # Группа команд
        commands_group = QGroupBox("Отправка команд")
        commands_layout = QVBoxLayout()
        
        # Поле для отправки команды для канала 1
        channel1_commands_layout = QHBoxLayout()
        channel1_commands_layout.addWidget(QLabel("Канал 1:"))
        
        self.channel1_on_cmd_input = QLineEdit()
        self.channel1_on_cmd_input.setPlaceholderText("Команда вкл (например: FF018800000089)")
        self.channel1_on_cmd_input.setText("FF018800000089")  # Значение по умолчанию
        channel1_commands_layout.addWidget(self.channel1_on_cmd_input)
        
        self.channel1_off_cmd_input = QLineEdit()
        self.channel1_off_cmd_input.setPlaceholderText("Команда выкл (например: FF010800000009)")
        self.channel1_off_cmd_input.setText("FF010800000009")  # Значение по умолчанию
        channel1_commands_layout.addWidget(self.channel1_off_cmd_input)
        
        self.send_channel1_on_btn = QPushButton("Вкл")
        self.send_channel1_on_btn.clicked.connect(lambda: self.send_custom_command(self.channel1_on_cmd_input.text()))
        self.send_channel1_on_btn.setEnabled(False)
        channel1_commands_layout.addWidget(self.send_channel1_on_btn)
        
        self.send_channel1_off_btn = QPushButton("Выкл")
        self.send_channel1_off_btn.clicked.connect(lambda: self.send_custom_command(self.channel1_off_cmd_input.text()))
        self.send_channel1_off_btn.setEnabled(False)
        channel1_commands_layout.addWidget(self.send_channel1_off_btn)
        
        commands_layout.addLayout(channel1_commands_layout)
        
        # Поле для отправки команды для канала 2
        channel2_commands_layout = QHBoxLayout()
        channel2_commands_layout.addWidget(QLabel("Канал 2:"))
        
        self.channel2_on_cmd_input = QLineEdit()
        self.channel2_on_cmd_input.setPlaceholderText("Команда вкл (например: FF010200000003)")
        self.channel2_on_cmd_input.setText("FF010200000003")  # Значение по умолчанию
        channel2_commands_layout.addWidget(self.channel2_on_cmd_input)
        
        self.channel2_off_cmd_input = QLineEdit()
        self.channel2_off_cmd_input.setPlaceholderText("Команда выкл (например: FF010400000005)")
        self.channel2_off_cmd_input.setText("FF010400000005")  # Значение по умолчанию
        channel2_commands_layout.addWidget(self.channel2_off_cmd_input)
        
        self.send_channel2_on_btn = QPushButton("Вкл")
        self.send_channel2_on_btn.clicked.connect(lambda: self.send_custom_command(self.channel2_on_cmd_input.text()))
        self.send_channel2_on_btn.setEnabled(False)
        channel2_commands_layout.addWidget(self.send_channel2_on_btn)
        
        self.send_channel2_off_btn = QPushButton("Выкл")
        self.send_channel2_off_btn.clicked.connect(lambda: self.send_custom_command(self.channel2_off_cmd_input.text()))
        self.send_channel2_off_btn.setEnabled(False)
        channel2_commands_layout.addWidget(self.send_channel2_off_btn)
        
        commands_layout.addLayout(channel2_commands_layout)
        
        # Поле для отправки команды для канала 3
        channel3_commands_layout = QHBoxLayout()
        channel3_commands_layout.addWidget(QLabel("Канал 3:"))
        
        self.channel3_on_cmd_input = QLineEdit()
        self.channel3_on_cmd_input.setPlaceholderText("Команда вкл (например: FF010020000021)")
        self.channel3_on_cmd_input.setText("FF010020000021")  # Значение по умолчанию
        channel3_commands_layout.addWidget(self.channel3_on_cmd_input)
        
        self.channel3_off_cmd_input = QLineEdit()
        self.channel3_off_cmd_input.setPlaceholderText("Команда выкл (например: FF010040000041)")
        self.channel3_off_cmd_input.setText("FF010040000041")  # Значение по умолчанию
        channel3_commands_layout.addWidget(self.channel3_off_cmd_input)
        
        self.send_channel3_on_btn = QPushButton("Вкл")
        self.send_channel3_on_btn.clicked.connect(lambda: self.send_custom_command(self.channel3_on_cmd_input.text()))
        self.send_channel3_on_btn.setEnabled(False)
        channel3_commands_layout.addWidget(self.send_channel3_on_btn)
        
        self.send_channel3_off_btn = QPushButton("Выкл")
        self.send_channel3_off_btn.clicked.connect(lambda: self.send_custom_command(self.channel3_off_cmd_input.text()))
        self.send_channel3_off_btn.setEnabled(False)
        channel3_commands_layout.addWidget(self.send_channel3_off_btn)
        
        commands_layout.addLayout(channel3_commands_layout)
        
        # Поле для отправки произвольной команды
        custom_cmd_layout = QHBoxLayout()
        custom_cmd_layout.addWidget(QLabel("Произвольная команда:"))
        
        self.custom_cmd_input = QLineEdit()
        self.custom_cmd_input.setPlaceholderText("Введите команду в HEX (например: FF018800000089)")
        custom_cmd_layout.addWidget(self.custom_cmd_input)
        
        self.send_custom_cmd_btn = QPushButton("Отправить")
        self.send_custom_cmd_btn.clicked.connect(lambda: self.send_custom_command(self.custom_cmd_input.text()))
        self.send_custom_cmd_btn.setEnabled(False)
        custom_cmd_layout.addWidget(self.send_custom_cmd_btn)
        
        commands_layout.addLayout(custom_cmd_layout)
    ...
    def update_connection_status(self):
        ...
        if self.connected:
            ...
            self.channel1_timer_start_btn.setEnabled(True)
            self.channel1_timer_stop_btn.setEnabled(True)
            self.channel2_timer_start_btn.setEnabled(True)
            self.channel2_timer_stop_btn.setEnabled(True)
            self.channel3_timer_start_btn.setEnabled(True)
            self.channel3_timer_stop_btn.setEnabled(True)
            self.start_all_timers_btn.setEnabled(True)
            self.stop_all_timers_btn.setEnabled(True)
    ...
    def send_custom_command(self, hex_command):
        """Отправить произвольную команду в HEX формате"""
        if not self.connected:
            self.log_message("Сначала подключитесь к устройству")
            return
        
        try:
            # Удалить пробелы и другие символы, оставить только шестнадцатеричные цифры
            clean_hex = ''.join(c for c in hex_command if c in '0123456789ABCDEFabcdef')
            
            # Проверить, что строка содержит четное количество символов (байты)
            if len(clean_hex) % 2 != 0:
                self.log_message(f"Ошибка: Неверный формат команды '{hex_command}' - должно быть четное количество шестнадцатеричных цифр")
                return
            ...
            self.log_message(f"Отправлена команда: {hex_command}")
            
        except ValueError:
            self.log_message(f"Ошибка: Неверный формат HEX команды '{hex_command}'")
        except Exception as e:
            self.log_message(f"Ошибка отправки команды '{hex_command}': {str(e)}")
