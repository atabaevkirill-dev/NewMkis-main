...
pub(crate) struct Mid {
    ...
    /// MKIS100TEST patch: the rest of this NAL is dropped (see `00 00 00 xx` below).
    discarding: bool,
}
            ...
            'body_byte: while let Some(next_byte) = cur.peek_byte() {
                ...
                if mid.seen_three_zeros {
                    match next_byte {
                        ...
                        _ => {
                            // MKIS100TEST patch. Emulation prevention rules out `00 00 00` inside
                            // a NAL, so what follows is not picture data: Beward B102S pads the
                            // last FU-A fragment of key frames with zeros and a few stray bytes.
                            // Upstream fails the whole session here; drop the rest of the NAL.
                            mid.seen_three_zeros = false;
                            mid.discarding = true;
                            break 'body_byte;
                        }
